...
class EditScenariosDialog(EditOrRemoveItemsDialog):
    """A dialog to query user's preferences for updating Scenarios."""
        ...
        for i in range(self.model.rowCount()):
            ...
            for database in db_names.split(","):
                for db_map in item.db_maps:
                    ...
                    return
            if not name:
                self.parent().msg_error.emit("Scenario name missing at row {}".format(i + 1))
                return
            orig_row = self.orig_data[i]
            if [name, description] == orig_row:
                continue
