...
class ParameterTagToolBar(QToolBar):
    ...
    def init_toolbar(self):
        ...
        self.actions = [action]
        self.db_map_tags = [[(db_map, {"id": 0}) for db_map in self.db_maps]]  # NOTE: 0 is 'untagged'
        tag_data = {}
        for db_map in self.db_maps:
            for parameter_tag in self.db_mngr.get_parameter_tags(db_map):
                tag_data.setdefault(parameter_tag["tag"], {})[db_map] = parameter_tag
        for tag, db_map_data in tag_data.items():
            action = QAction(tag)
            self.insertAction(self.empty_action, action)
            action.setCheckable(True)
            button = self.widgetForAction(action)
            self.tag_button_group.addButton(button, id=len(self.db_map_tags))
            self.actions.append(action)
            self.db_map_tags.append([(db_map, tag) for db_map, tag in db_map_data.items()])
        self.tag_button_group.buttonToggled["int", "bool"].connect(
            lambda i, checked: self.tag_button_toggled.emit(self.db_map_tags[i], checked)
        )
    ...
    def _add_db_map_tag_actions(self, db_map, parameter_tags):
        ...
        for parameter_tag in parameter_tags:
            if parameter_tag["tag"] in action_texts:
                ...
                self.db_map_tags[i].append((db_map, parameter_tag))
            else:
                ...
                self.tag_button_group.addButton(button, id=len(self.db_map_tags))
                self.actions.append(action)
                self.db_map_tags.append([(db_map, parameter_tag)])
    ...
    def receive_parameter_tags_removed(self, db_map_data):
        for db_map, parameter_tags in db_map_data.items():
            self._remove_db_map_tag_actions(db_map, parameter_tags)

    def _remove_db_map_tag_actions(self, db_map, parameter_tags):
        for tag in parameter_tags:
            i = next(k for k, x in enumerate(self.db_map_tags) if (db_map, tag) in x)
            self.db_map_tags[i].remove((db_map, tag))
            if not self.db_map_tags[i]:
                self.db_map_tags.pop(i)
    ...
    def _update_db_map_tag_actions(self, db_map, parameter_tags):
        for parameter_tag in parameter_tags:
            i = next(k for k, x in enumerate(self.db_map_tags) if (db_map, parameter_tag) in x)
