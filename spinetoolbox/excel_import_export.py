...
def validate_sheet(ws):
    ...
    Returns:
        True if sheet is valid, False otherwise
    """
    sheet_type = ws['A2'].value
    sheet_data = ws['B2'].value
...
def read_relationship_sheet(ws):
    ...
    data_parameter = []
    if num_parameters > 0:
        relationship_object_classes + parameters  # TODO: This line does not do anything
...
def export_object_parameter_values(db, data):
    ...
    objects = set([c[1].object for c in parameter_values])  # TODO: This is not used anywhere
...
def export_relationships_to_spine_db(db, data):
    ...
    objects = [item for sublist in objects for item in sublist]
    objects = set([item for sublist in objects for item in sublist])  # TODO: Is 'objects' local variable used anywhere?
