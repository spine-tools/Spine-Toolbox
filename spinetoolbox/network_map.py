...
Importera från SPINE
GUI SPINE, ska då inte gå att göra fel (ordning metoder, variabler etc.)
Variabelnamn (ändra sen när detta är bestämt)

sub, div mer generiska (kör på alla node och conn-variabler utom vissa typ index,x,y)
...
class NetworkMap:
    ...
    def __init__(self, parent, mapping, settings_path=""):
        """ Initialize class."""
        super().__init__()
        ...
        self.N = None # len(parent.node_name) # number of nodes
        self.node = None # np.arange(self.N) # node index
        self.node_status = None # parent.node_status
        self.node_info = None # parent.node_info # additional info to be displayed in interactive mode
        self.node_parameter_dict = None # parent.node_parameter # dict with parameter name and array
        self.node_info_param = None # s['node_info_parameter'] # parameters to be displayed in interactive mode
        # conn data
        self.from_node_index_arr = None # mult_ind(parent.from_node, self.node_name_list) # index for from node
        self.to_node_index_arr = None # mult_ind(parent.to_node, self.node_name_list) # index for to node
        self.conn_name_list = None # parent.conn_name
        self.conn_status = None # parent.conn_status
        self.conn_info = None # parent.conn_info # additional info to be displayed in interactive mode
        self.conn_parameter_dict = None # parent.conn_parameter # dict with parameter name and array
    ...
    def init_network_data(self):
        ...
        print(self.from_node_index_arr)
        print(self.to_node_index_arr)
        ...
        #unit_info = mult_data(relationship_data[relationship_id==relationship_class_unit_node,unit_col].astype(int),unit_id,unit_name)
        #unit_node_ind = mult_data(relationship_data[relationship_id==relationship_class_unit_node,node_col].astype(int),node_id,
        #                          np.arange(len(node_id)))
    ...
    def default_settings(self):
        ...
        settings = {}
        settings['node_info_parameter'] = [] # For now # self.node_parameter_keys.copy() # parameters to be displayed in interactive mode
    ...
    def calculate_layout(self):
        ...
        # Possibly swich dimensions (default is landscape-like orientation)
    ...
    def heavy_data(self,heavy,bus):
        ...
        h_ind = mult_ind([k for k in heavy.keys()],bus) # indeces for buses
    ...
    def set_map_properties(self):
        ...
        gp = max(self.N, 300) * 100 # ~100*N grid points in heatmaps
